...
namespace DevExpress.XtraRichEdit.Design {
    ...
    public class RepositoryItemRichTextIndentEdit : RepositoryItemBaseSpinEdit {
        ...
        protected internal override MaskManager CreateMaskManager(MaskProperties mask) {
            RichTextIndentEdit indentEdit = OwnerEdit as RichTextIndentEdit;
            UIUnitConverter unitConverter = indentEdit?.UIUnitConverter ?? new UIUnitConverter(UnitPrecisionDictionary.DefaultPrecisions);
        ...
        public override void Assign(RepositoryItem item) {
            ...
            try {
                ...
                DefaultUnitType = source.DefaultUnitType;
                //this.IsValueInPercent = source.IsValueInPercent;
                MinValue = source.MinValue;
                MaxValue = source.MaxValue;
                DefaultValue = source.DefaultValue;
                AllowAssignExceedLimits = source.AllowAssignExceedLimits;
                DisplayValueAlwaysAsPositive = source.DisplayValueAlwaysAsPositive;
            }
            finally {
                EndUpdate();
            }
        ...
        void OnCustomDisplayText(object sender, XtraEditors.Controls.CustomDisplayTextEventArgs e) {
            e.DisplayText = TryMakePositiveDisplayText(e.DisplayText, DisplayValueAlwaysAsPositive);
        }
    }
#if DEBUGTEST
    [DXToolboxItem(true)]
    ...
    public class RichTextIndentEdit : BaseSpinEdit, IBatchUpdateable, IBatchUpdateHandler {
        ...
        public RichTextIndentEdit() {
            ...
            uiUnitConverter = new UIUnitConverter(GetUnitPrecisions());
            valueUnitConverter = new DocumentModelUnitTwipsConverter();
        ...
        public virtual int? Value {
            ...
            set {
                float? newValue = (value == null) ? value : (float?)ValueUnitConverter.ModelUnitsToTwipsF(value.Value);
                CoreValue = newValue;
                exactValue = value;
                if(!newValue.HasValue)
                    return;
                if(!Properties.AllowAssignExceedLimits && !Properties.IsWithinAllowedLimits((int)Math.Round(newValue.Value)))
                    exactValue = ValueUnitConverter.TwipsToModelUnits(Properties.MinValue);
            }
        }
